...
{
    const BSF_FIELDS_SET_NAME = "Technical";
    const BSF_FIELD_ITEM_TYPE_NAME = "Item type";
    const BSF_FIELD_ITEM_TYPE_ID = "ItemType";
    const BSF_FIELD_COLLECTION_NAME = "Collection";
    const BSF_FIELD_COLLECTION_ID = "Collection";
    ...
    private $_autodetectItemTypeCollection;
    ...
    public function init()
    {
        ...
        if ($this->_autodetectItemTypeCollection) {
            $elementsByElementSetName += array(__(self::BSF_FIELDS_SET_NAME) => array(
                self::BSF_FIELD_ITEM_TYPE_ID => __(self::BSF_FIELD_ITEM_TYPE_NAME),
                self::BSF_FIELD_COLLECTION_ID => __(self::BSF_FIELD_COLLECTION_NAME)
            ));

            $multipleElems = array();
            $itemTypes = get_db()->getTable('ItemType')->findPairsForSelectForm();
            // We search for different fields of the same elem type for various item types
            foreach ($itemTypes as $itemType => $itemName) {
                $elemsForType = $this->_getElementPairs($itemType);
                foreach ($elemsForType as $elemId => $elem) {
                    if (!array_key_exists($elemId, $multipleElems)) {
                        $multipleElems[$elemId] = array();
                    }

                    $foundSameElemIndex = -1;
                    foreach ($multipleElems[$elemId] as $i => $sameElemId) {
                        if ($sameElemId["elem"] == $elem) {
                            $foundSameElemIndex = $i;
                            break;
                        }
                    }

                    if ($foundSameElemIndex == -1) {
                        $multipleElems[$elemId][] = array(
                            "elem" => $elem,
                            "itemTypes" => array()
                        );
                        $foundSameElemIndex = count($multipleElems[$elemId]) - 1;
                    }

                    $multipleElems[$elemId][$foundSameElemIndex]["itemTypes"][] = $itemName;
                }
            }
            foreach ($multipleElems as $elemName => $elemData) {
                if (count($elemData) == 1) {
                    $elementsByElementSetName += array(
                        $elemName => $elemData[0]["elem"]
                    );
                } elseif (count($elemData) > 1) {
                    foreach ($elemData as $elemDatum) {
                        $elementsByElementSetName += array(
                            $elemName." - ".join(" - ", $elemDatum["itemTypes"]) => $elemDatum["elem"]
                        );
                    }
                }
            }
        ...
        if (strlen($columnNameDelimiter) > 0) {
            if ($columnNameParts = explode($columnNameDelimiter, $columnName)) {
                if (count($columnNameParts) == 2) {
                    ...
                    if ($elementSetName == __(self::BSF_FIELDS_SET_NAME)) {
                        if ($elementName == self::BSF_FIELD_ITEM_TYPE_ID || $elementName == __(self::BSF_FIELD_ITEM_TYPE_NAME)) {
                            $element = array(
                                "id" => self::BSF_FIELD_ITEM_TYPE_ID
                            );
                        } elseif ($elementName == self::BSF_FIELD_COLLECTION_ID || $elementName == __(self::BSF_FIELD_COLLECTION_NAME)) {
                            $element = array(
                                "id" => self::BSF_FIELD_COLLECTION_ID
                            );
                        }
                    } else {
     ...
     * @param int $elementDelimiter The element delimiter
     */
    public function setAutodetectItemTypeCollection($autodetectItemTypeCollection)
    {
        $this->_autodetectItemTypeCollection = $autodetectItemTypeCollection;
    }
    ...
    */
    ...
    protected function _getColumnMap($index, $columnName)
    {
        ...
        foreach($elementIds as $elementId) {
            ...
            if ($elementId == "ItemType") {
                $elementMap = new CsvImport_ColumnMap_ItemType($columnName);
            } elseif ($elementId == "Collection") {
                $elementMap = new CsvImport_ColumnMap_Collection($columnName);
            } else {
